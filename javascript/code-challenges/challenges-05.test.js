...
const addValues = (arr) => {
  // Solution code here...
  // let arr=[];
  let red=arr.reduce((acc,val,idx)=>{
return acc=acc+val
  },0)
  return red
};
...
const returnNames = (arr) => {
  // Solution code here...
  let arr2=[]
  let name0 =arr.reduce((acc,val,idx)=>{
    acc[idx]=val.name
    return acc
  },[])
  return name0
...
const reversedString = (str) => {
  ...
  console.log(arr);
  let revers=str.reduce((acc,val,idx)=>{
...
let count2=count.reduce((acc,val,idx)=>{
if(val.children){
...
const extractChildren = (arr) => {
  ...
  let arr1=arr.filter((val,idx)=>{
    ...
    let arr2=[]
  let l=val.children[0];
  console.log(l)
  // for(let i=0;i<l;i++){
  //   if(val.children[i].includes('a')){
      
  //    arr2.push(val.children)
  //   }
  // }
    // return arr2[0]
