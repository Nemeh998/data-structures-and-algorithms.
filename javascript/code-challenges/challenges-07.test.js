...
const listFoods = (recipe) => {
  ...
  // Solution code here...
  let ele=recipe.ingredients.map(e=>{
return e.split(' ').splice(2,3);
  }).map(ele1=>{
return ele1.join(' ');
  });
...
const stepActions = (recipe) => {
  let ele=recipe. steps.map(e=>{
  return e.split(' ').splice(0,1);
      }).map(ele1=>{
    return ele1.join(' ');
      });
    // console.log(ele);
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  // console.log(str)
// let A=str.map(x=>{
  let divid=str.split('').reverse().slice(numberOfCharacters).reverse().join('');
  console.log(divid);
  // console.log(x)
// });
//   console.log(A)
...
//     console.log(typeof(Number(str[i])));
...
Write a function named removeVowels that takes in a 5101
string and returns a new string where all the vowels of the original string have been removed.
...
Write a function named extractVowels that takes in a string and returns an array where the first element is the original 
string with all the vowels removed, and the second element is a string of all the vowels that were removed, in alphabetical order.
