...
const maxInArray = (arr) => {
  // Solution code here...
  const resuce=arr.reduce((acc,val)=>{
if(acc>val){
    return acc;
}else{
    return val;
...
let reslut=[];
   arr.forEach(val => {
       if(val.name===character){
           let index=Object.keys(val).indexOf('children');
           if(index>=0){
reslut=Object.values(val)[index];
}
...
return reslut.length?true:false;
};
